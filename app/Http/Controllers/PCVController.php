...
public function index(Request $request) {
    ...
    try {
        ...
        $results = DB::select(
            'EXEC sproc_PHP_PCV @mode = ?, @params = ?',
            ['get' ,$params] 
        );
...
public function upsert(Request $request)
{
        ...
        try {
            ...
            $result = DB::select('EXEC sproc_PHP_PCV @mode = ?, @params = ?', [
                $mode,
                $params
            ]);
...
public function finalize(Request $request) {
    ...
    try {
       ...
       $validated = $request->validate([
        ...
        $results = DB::select(
            'EXEC sproc_PHP_Posting_PCV @mode = ?, @params = ?',
            ['Finalize', $params]
        );
...
public function generateGL(Request $request)
    {
        try {
            ...
            $results = DB::select("EXEC sproc_PHP_PCV @mode = ?, @params = ?", [
                'GenerateEntries',
                $jsonString
            ]);
            ...
            Log::error('Error executing sproc_PHP_PCV: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
