...
class RMAJController extends Controller
{
...
public function index(Request $request) {
    ...
    try {
        ...
        $results = DB::select(
            'EXEC sproc_PHP_RMAJ @mode = ?, @params = ?',
            ['get' ,$params] 
        );
...
public function upsert(Request $request)
{
        ...
        try {
            ...
            $result = DB::select('EXEC sproc_PHP_RMAJ @mode = ?, @params = ?', [
                $mode,
                $params
            ]);
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'Error executing RMAJ Upsert.',
...
public function finalize(Request $request) {
    ...
    try {
       ...
       $validated = $request->validate([
        ...
        $results = DB::select(
            'EXEC sproc_PHP_Posting_RMAJ @mode = ?, @params = ?',
            ['Finalize', $params]
        );
...
public function generateGL(Request $request)
    {
        try {
            ...
            $results = DB::select("EXEC sproc_PHP_RMAJ @mode = ?, @params = ?", [
                'GenerateEntries',
                $jsonString
            ]);
            ...
            Log::error('Error executing sproc_PHP_RMAJ: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
...
public function validateUpload(Request $request)
{
    ...
    try {
        ...
        Log::error('Error validating RMAJ upload: ' . $e->getMessage());

        return response()->json([
            'status' => 'error',
            'message' => 'Error validating RMAJ upload.',
            'details' => $e->getMessage()
        ], 500);
    }
}
