...
public function index(Request $request) {
    ...
    try {
        ...
        $results = DB::select(
            'EXEC sproc_PHP_ARCM @mode = ?, @params = ?',
            ['get' ,$params] 
        );
...
public function upsert(Request $request)
{
        ...
        try {
            ...
            $result = DB::select('EXEC sproc_PHP_ARCM @mode = ?, @params = ?', [
                $mode,
                $params
            ]);
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'Error executing ARCM Upsert.',
...
public function finalize(Request $request) {
    ...
    try {
       ...
       $validated = $request->validate([
        ...
        $results = DB::select(
            'EXEC sproc_PHP_Posting_ARCM @mode = ?, @params = ?',
            ['Finalize', $params]
        );
...
public function generateGL(Request $request)
    {
        try {
            ...
            $results = DB::select("EXEC sproc_PHP_ARCM @mode = ?, @params = ?", [
                'GenerateEntries',
                $jsonString
            ]);
            ...
            Log::error('Error executing sproc_PHP_ARCM: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
