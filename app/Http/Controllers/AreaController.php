...
class AreaController extends Controller
{
    ...
    {
        $request->validate(['AREA_CODE' => 'required|string']);
        try {
            $results = DB::select(
                'EXEC sproc_PHP_AreaRef @mode = ?, @params = ?',
                ['Get', $request->AREA_CODE] // FIXED: Was BILLTERM_CODE
            );
    ...
    public function delete(Request $request)
    {
        ...
        $code = $data['areaCode'] ?? null; // FIXED: Was billtermCode

        if (!$code) {
            return response()->json(['success' => false, 'message' => 'Area code is required.'], 400);
        }

        try {
            $params = json_encode(['json_data' => $data]);
            DB::statement('EXEC sproc_PHP_AreaRef @mode = ?, @params = ?', ['Delete', $params]);
